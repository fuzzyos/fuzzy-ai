...
```typescript
import { getImageModel, generateImages } from '@fuzzyos/fuzzy-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are fuzzy thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
